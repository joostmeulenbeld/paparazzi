...
float chirp_update(struct chirp_t* chirp, float current_time_s) {
  ...
  // If fade_in=true but this point is reached, the current time t is the time since the fade-in stopped
  if (chirp->fade_in)
    t -= 2 / chirp->f0_hz;

  if (chirp->exponential_chirp) { // See the book referenced in sys_id_chirp.h for the equations
